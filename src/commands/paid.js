    ...
    for (let i = 0, cancel = false; i < questions.length && cancel === false; i++) {
        ...
        if (questionMsg == null) {
            ...
            for (let x = 0, cancel = false; x < selectedQuestions.length && cancel === false; x++) {
                ...
                // Send advert
                if (answers.finish == 1) {
                    tryToSendChannelId(client, process.env.ADVERT_PAID_ID, advertToEmbedPaid(answers, user));
                    tryToSendChannelId(client, process.env.SHARED, advertToEmbedPaid(answers, user));
                }
                if (answers.finish == 2) {
                    answers.finish = null;
                    x = -1;
